...
function loadEFIDisk_WU() {
  function checkDriverLetter(dl) {
    ...
    window.services.checkDriveLetter(diskno).then(
      ...
      function () {
        window.services.loadEFIDisk(diskno).then(function () {
            showTipModal(fillLangString(VUEAPP.lang.tip_mount_disk_success,diskno));
          },function () {
      ...
      for (let i = 0; i < arrayRes.length; i++) {
        ...
        if (itval.length === 5 && itval[2].indexOf("EFI") === 0) {
          jq_select_efi_disk.append(
            `<option value='${itval[4]}'>${itval[2] + "_" + itval[4]}</option>`
          );
        }
      }
    },
...
const vueproperty = {
  data() {
    return {
      ...
      OCbuttons: {
        buttons5: ["enabled", "copy", "paste", "add", "del"],
        buttons4: ["copy", "paste", "add", "del"],
      },
    };
  },
  ...
  watch: {
    ...
    configisfull(newval, oldval) {
      if (
        oldval === true &&
        newval === false &&
        $("#li_PlatformInfo_Generic").hasClass("active") === false
      ) {
  ...
  methods: {
    ...
    initNVRAM: function () {
      ...
      //Add
      this.getPlistAndResetTableDataForValue(
        plistData,
        this.NVRAM,
        "Add",
        "NVRAM_Add"
      ...
      //Delete
      this.getPlistAndResetTableDataForVolume(
        plistData,
        this.NVRAM,
        "Delete",
        "NVRAM_Delete"
    ...
    initUEFI: function () {
      ...
      //ProtocolOverrides
      this.getAndSetDictItem(
        plistData.ProtocolOverrides,
        this.UEFI.ProtocolOverrides
      );
    ...
    initPlatformInfo: function () {
      ...
      //PlatformNVRAM
      this.getAndSetDictItem(
        plistData.PlatformNVRAM,
        this.PlatformInfo.PlatformNVRAM
      );
      ...
      this.getPlistAndResetTableData(
        ...
        this.PlatformInfo.Memory_Devices
      );
    ...
    initMisc: function () {
      ...
      for (let i = 0, len = arrayBlessOverride.length; i < len; i++) {
        this.Misc.BlessOverride.push({
          ScanningPaths: arrayBlessOverride[i][0],
        });
      }
    ...
    initDeviceProperties: function () {
      //Add
      this.getPlistAndResetTableDataForValue(
        this.plistJsonObject.DeviceProperties,
    ...
    initACPI: function () {
      ...
      this.configisMOD =
        plistData.Quirks?.EnableForAll === undefined ? false : true;
     ...
     */
    getPlistAndResetTableData: function (
      plistData,
      lastkey,
      gridkey,
      gridData
    ...
    getPlistAndResetTableDataForValue: function (
      plistData,
      vueData,
      key,
      tablename
      ...
      if (keypddata) {
        ...
        showTipModal(
          fillLangString(VUEAPP.lang.plistformaterror, key),
          "warning"
        );
      }
    ...
    getPlistAndResetTableDataForVolume: function (
      plistData,
      vueData,
      key,
      tablename
    ...
    btncheckboxclick: function (event, vlen = 8) {
      ...
      for (let i = piv16.length - 1, k = 1; i >= 0; i--, k++) {
        ...
        for (let j = 0; j < itval.length; j++) {
          this.Assist.pagePublic_Selected.push(
            "0x" + "0".repeat(vlen - k) + itval[j] + "0".repeat(k - 1)
          );
        }
      }
    ...
    checkboxPageBtnOKclick: function () {
      if (this.Assist.RADIO_CHECK_BOX === "C") {
        this[this.Assist.last_checkbox_ids[1]][
          this.Assist.last_checkbox_ids[2]
        ][this.Assist.last_checkbox_ids[3]] = this.getCheckedTotal();
      } else if (this.Assist.RADIO_CHECK_BOX === "R") {
        this[this.Assist.last_radiobox_ids[1]][
          this.Assist.last_radiobox_ids[2]
        ][this.Assist.last_radiobox_ids[3]] =
          this.Assist.pageRadio_CurrentValue;
      }
    ...
    getCheckedTotal: function () {
      let pagetotal = 0;
      for (
        let i = 0, len = this.Assist.pagePublic_Selected.length;
        i < len;
        i++
      ) {
    ...
    imgButtonClick: function (event) {
      ...
      this["imgButtonClick_" + buttonid.substring(0, buttonid.indexOf("_"))](
        buttonBehind,
        currentGridTable
      );
    },
    ...
    imgButtonClick_btnenabled: function (buttonBehind, currentGridTable) {
      ...
      if (selectedIds.length > 0) {
        let theEnabled = currentGridTable.jqGrid(
          "getCell",
          selectedIds[0],
          "Enabled"
        );
    ...
    imgButtonClick_btnadd: function (buttonBehind, currentGridTable) {
      if (buttonBehind.endsWith("Right")) {
        const theGrid = getJqgridObjectbyKey(
          buttonBehind.replace("Right", "Left")
        );
    ...
    imgButtonClick_btndel: function (buttonBehind, currentGridTable) {
      ...
      if (selectedIds.length > 0) {
        ...
        for (let it of deleteIds) {
          ...
          GLOBAL_SET_ONEDITTABLE.delete(
            "#gridtable_" + buttonBehind + "_" + it
          );
        ...
        if (buttonBehind.endsWith("Left")) {
          ...
          for (let it of rowIds) {
            ...
            GLOBAL_SET_ONEDITTABLE.delete(
              "#gridtable_" + rightButtonBehind + "_" + it
            );
    ...
    imgButtonClick_btncopy: function (buttonBehind, currentGridTable) {
      ...
      if (buttonBehind.endsWith("Right")) {
        const leftGrid = getJqgridObjectbyKey(
          buttonBehind.replace("Right", "Left")
        );
...
const VUEAPP = Vue.createApp(vueproperty).mount("#main-container");
...
function savePlistUtools() {
  ...
  //保存文件
  const savefile = window.services.saveFile($("#span_footermessage").text(), xmlcontext);
  savefile.then(
    function (res) {
